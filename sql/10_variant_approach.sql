-- ============================================================================
-- Ultimate Fix: SP_AGENT_PROFILE with JSON Escape Solution
-- ============================================================================
-- This version uses a different approach to avoid JSON escaping issues
-- ============================================================================
...
-- Drop and recreate the procedure with JSON escape solution
...
def sp_agent_profile(session, stage_path, sample_size=10000, data_dictionary_ref=None, file_format='CSV_FORMAT'):
    """
    REAL Profiling agent implementation with JSON escape solution.
    """
    ...
    try:
        ...
        # Step 6: Save to database using VARIANT approach
        file_name = stage_path.split('/')[-1] if '/' in stage_path else stage_path
        
        try:
            # Use VARIANT approach to avoid JSON escaping issues
            # First, create a temporary table with VARIANT columns
            temp_table_sql = f"""
            CREATE OR REPLACE TEMPORARY TABLE temp_profiling_json (
                profile_id STRING,
                schema_data VARIANT,
                stats_data VARIANT,
                pii_data VARIANT,
            ...
            """
            session.sql(temp_table_sql).collect()
            
            # Insert data using VARIANT constructor
            temp_insert_sql = f"""
            INSERT INTO temp_profiling_json VALUES (
                '{profile_id}',
            ...
            """
            session.sql(temp_insert_sql).collect()
            ...
            SELECT 
                ...
                summary_text,
                CURRENT_TIMESTAMP()
            FROM temp_profiling_json
            WHERE profile_id = '{profile_id}'
            """
            session.sql(main_insert_sql).collect()
            
            # Also try to insert into PROFILING_RESULTS
            try:
                profiling_insert_sql = f"""
                ...
                WHERE profile_id = '{profile_id}'
                """
                session.sql(profiling_insert_sql).collect()
...
SELECT 'Updated SP_AGENT_PROFILE procedure with VARIANT approach!' AS STATUS;
